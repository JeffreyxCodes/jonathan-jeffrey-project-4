...
app.getRandomArray = function (possibleIndexes, arrayLength, mustContain) {
    ...
    if (mustContain) { // if mustContain is given make sure it is in there
        // remove the mustContain from orderedArray to have it added randomly last
        orderedArray.splice(mustContain, 1);
        for (let i = 1; i < arrayLength; i++) {
            randomArray.push(orderedArray.splice(Math.floor(Math.random() * orderedArray.length), 1)[0]);
        }
        // add the mustContain randomly into the randomArray
        randomArray.splice(Math.floor(Math.random() * arrayLength), 0, mustContain);
    } else { // if mustContain is not given then the values of the array would be absolutely random
...
app.populateDropDown = function () {
    ...
    let options = app.getRandomArray(songArray.length, app.totalOptions, app.currentSongIndex);

    options = options.map(index => {
        return `<option value="${songArray[index].track}">"${songArray[index].track}" by ${songArray[index].artist}</option>>`;
    });
